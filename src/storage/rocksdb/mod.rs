...
impl RocksDBStorageTables {
    ...
    pub fn update_gas_coins(
        ...
        _metrics: &Arc<StoragePoolMetrics>,
    ) -> anyhow::Result<()> {
        let group_keys = updated_gas_coins
            .iter()
    ...
    fn iter_available_gas_coins(
        &self,
    ) -> impl Iterator<Item = ((SuiAddress, ObjectID), GasCoinInfo)> + '_ {
        self.available_gas_coins.unbounded_iter()
    }

    #[cfg(test)]
    fn iter_reserved_gas_groups(
        &self,
    ) -> impl Iterator<Item = (GasGroupKey, ReservedGasGroup)> + '_ {
        self.reserved_gas_groups.unbounded_iter()
    }

    #[cfg(test)]
    fn iter_expiration_queue(
        &self,
    ) -> impl Iterator<Item = (SuiAddress, ExpirationTimeMs, GasGroupKey)> + '_ {
        self.expiration_queue.unbounded_iter().map(|(key, _)| key)
    }

    #[cfg(test)]
    fn iter_pending_update_gas_groups(
        &self,
    ) -> impl Iterator<Item = (GasGroupKey, ReservedGasGroup)> + '_ {
        self.pending_update_gas_groups.unbounded_iter()
    }

    #[cfg(test)]
    fn check_table_consistency(&self) {
        let valid_addresses = self.iter_all_valid_addresses();
        let available_gas_coins: HashMap<_, _> = self.iter_available_gas_coins().collect();
        let mut updated_available_gas_coins = HashMap::new();
        for ((addr, id), coin) in available_gas_coins.into_iter() {
            assert!(updated_available_gas_coins
                .insert(id, (addr, coin))
                .is_none());
        }
        let reserved_gas_groups: HashMap<_, _> = self.iter_reserved_gas_groups().collect();
        let pending_update_gas_groups: HashMap<_, _> =
            self.iter_pending_update_gas_groups().collect();
        let expiration_queue: HashSet<_> = self.iter_expiration_queue().collect();
        let mut updated_expiration_queue = HashMap::new();
        for (addr, exp_time, group_key) in expiration_queue.into_iter() {
            assert!(updated_expiration_queue
                .insert(group_key, (addr, exp_time))
                .is_none());
        }
        assert_eq!(
            updated_expiration_queue.keys().collect::<BTreeSet<_>>(),
            reserved_gas_groups.keys().collect::<BTreeSet<_>>()
        );

        for (id, (addr, _)) in updated_available_gas_coins.iter() {
            assert!(valid_addresses.contains(addr));
            assert!(!reserved_gas_groups.contains_key(id));
            assert!(!pending_update_gas_groups.contains_key(id));
            assert!(!updated_expiration_queue.contains_key(id));
        }
        for (group_key, group) in reserved_gas_groups.iter() {
            assert_eq!(group.get_key(), *group_key);
            assert!(!pending_update_gas_groups.contains_key(group_key));

            let (address, exp_time) = updated_expiration_queue.get(group_key).unwrap();
            assert_eq!(exp_time, &group.expiration_time);
            assert!(valid_addresses.contains(address));

            assert!(!updated_available_gas_coins.contains_key(group_key));
            for object in group.objects.iter() {
                assert!(!updated_available_gas_coins.contains_key(object));
            }
        }
        for (group_key, group) in pending_update_gas_groups.iter() {
            assert_eq!(group.get_key(), *group_key);
            assert!(!reserved_gas_groups.contains_key(group_key));
            assert!(!updated_expiration_queue.contains_key(group_key));
...
impl Storage for RocksDBStorage {
    ...
    async fn get_available_coin_count(&self) -> usize {
        let _guard = self.mutexes.write();
        self.tables.iter_available_gas_coins().count()
    }
    ...
    async fn get_pending_update_coin_count(&self) -> usize {
        let _guard = self.mutexes.write();
        self.tables
            .iter_pending_update_gas_groups()
            .map(|(_, group)| group.objects.len())
            .sum()
    }
    ...
    async fn check_table_consistency(&self) {
        let _guard = self.mutexes.write();
        self.tables.check_table_consistency();
    }
}
