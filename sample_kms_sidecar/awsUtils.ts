...
async function getSerializedSignature(signature: Uint8Array, sui_pubkey: Secp256k1PublicKey) {
...
export async function signAndVerify(tx_bytes: Uint8Array) {
  ...
  try {
    ...
    const publicKeyToUse = original_pubkey instanceof Secp256k1PublicKey ? original_pubkey : undefined;
    ...
    if (publicKeyToUse !== undefined) {
      ...
      const isValid = await publicKeyToUse.verifyTransactionBlock(tx_bytes, serializedSignature);
